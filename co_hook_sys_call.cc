...
typedef ssize_t (* recvfrom_pfn_t)(int socket,void* message,size_t length,int flags,struct sockaddr* address,socklen_t address_len);
typedef ssize_t (* send_pfn_t)(int socket,const void* message,size_t length,int flags);
typedef ssize_t (* recv_pfn_t)(int socket,void* message,size_t length,int flags);
...
typedef int (* gethostbyname_r_pfn_t)(const char * __restrict name,struct hostent * __restrict ret, char * __restrict buf, size_t buflen,
                                                                                      struct hostent ** __restrict result, int * __restrict h_errnop);

//RTLD_NEXT代表这里获取到的是系统库中的函数,如果是RTLD_DEFAULT则表示的是获取到的是我们自己定义的函数
...
static __poll_pfn_t s_sys__poll_func = (__poll_pfn_t)dlsym(RTLD_NEXT,"__poll");
...
static inline rpchook_t* alloc_by_fd(int fd){
    if(fd > -1 && ((int)sizeof(g_rpchook_socket_fd) / (int)sizeof(g_rpchook_socket_fd[0]))){
...
int connect(int fd,const struct sockaddr* address,socklen_t address_len){
    ...
    //如果是用户自己设置的非阻塞，那么也不用管了，交给用户自己去完成
    ...
    //wait
   //执行到这里就代表fd是非阻塞的并且connect还未成功就返回 
   int pollret = 0;
   struct pollfd pf = {0};
   for(int i = 0;i < 3;++i){
       //重复三次，每一个25s，一共75s
...
ssize_t read(int fd,void* buf,size_t nbyte){
    ...
    //最终调用的是co_poll_inner,里面会将写事件注册到epoll里面去，并且会yield出当协程,一直知道超时/注册的事件发生时，才会resume回来
...
ssize_t recvfrom(int socket,void* buffer,size_t length,int flags,struct sockaddr* address,socklen_t address_len){
    ...
    struct pollfd pf = {0};
    pf.fd = socket;
    pf.events = (POLLIN | POLLERR | POLLERR);
...
ssize_t send(int socket,const void* buffer,int length,int flags){
    HOOK_SYS_FUNC(send);
    if(!co_is_enable_sys_hook()){
    ...
    size_t wrotelen = 0;
    int timeout = (lp->write_timeout.tv_sec * 1000) + (lp->write_timeout.tv_usec / 1000);
    size_t writeret = g_sys_send_func(socket,buffer,length,flags);
    if(writeret == 0){
        return writeret;
    }
    if(writeret > 0){
    ...
    while(wrotelen < length){
        ...
        poll(&pf,1,timeout);
        //
        writeret = g_sys_send_func(socket,buffer,length,flags);
        if(writeret <= 0){
            break;
        }
        wrotelen += writeret;
...
int setsockopt(int fd,int level,int option_name,const void* option_value,socklen_t option_len){
    ...
    if(lp && SOL_SOCKET == level){
        //如果是hook的socket并且level == SOL_SOCLET的话，需要额外做的操作
        //更新超时时间
        struct timeval* val = (struct timeval*)option_value;
        if(SO_RCVTIMEO == option_name){
            memcpy(&lp->read_timeout,val,sizeof(*val));
        }
        if(SO_SNDTIMEO == option_name){
...
int fcntl(int fildes,int cmd,...){
    ...
    switch(cmd){
        ...
        case F_SETFL:
            {
                ...
                if(co_is_enable_sys_hook() && lp){
                    //如果当前协程开启了hook并且操作的fd属于是hook的socket
                    flag |= O_NONBLOCK;
                }
                ret = g_sys_fcntl_func(fildes,cmd,flag);
                if(0 == ret && lp){
                    //O_NONBLOCK是自动设置的，不属于是用户设置的标志
                   lp->user_flag = param;
                }
                break;
            }
        ...
        case F_GETLK:
            {
                struct flock* param = va_arg(ap,struct flock*);
                ret = g_sys_fcntl_func(fildes,cmd,param);
                break;
            }
        case F_SETLK:
...
static stCoSysEnvArr_t* dup_co_sysenv_arr(stCoSysEnvArr_t* arr){
    stCoSysEnvArr_t* lp = (stCoSysEnvArr_t*)calloc(1,sizeof(stCoSysEnvArr_t));
    if(arr->cnt){
        lp->data = (stCoSysEnv_t*)calloc(1,sizeof(stCoSysEnv_t) * arr->cnt);
        lp->cnt = arr->cnt;
...
//创建res_state_wrap类型的statis变量，并且是线程私有变量，名称为__co_state_wrap
...
struct hostent* co_gethostbyname(const char* name){
    ...
    while(ret = gethostbyname_r(name,host,__co_hostbuf_wrap->buffer,
                                   __co_hostbuf_wrap->iBufferSize,&result,h_errnop) == ERANGE && *h_errnop == NETDB_INTERNAL){
        free(__co_hostbuf_wrap->buffer);
